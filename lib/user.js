...
User.prototype.buy = function(productName, date, cb){
	var INSERT_SALE = 'INSERT INTO shoppingdb.sale(userId, productName, date) VALUES'
		+ '(' + this.id + ', "' + productName + '", "' +date + '")';
	database.query(INSERT_SALE, cb);
};

User.prototype.unBuy = function(productName, cb){
	var DELETE_SALE = 'DELETE FROM shoppingdb.sale WHERE userId = ' +this.id;
	database.query(DELETE_SALE, cb);
};

User.prototype.getListProducts = function(cb){
	var SELECT_PRODUCTS = 'SELECT p.name, p.price, s.date FROM '
		+ 'shoppingdb.product p INNER JOIN shoppingdb.sale s '
		+ 'ON p.name = s.productName '
